...
const drawFace = (ctx: CanvasRenderingContext2D, p: any, levelIdx: number) => {
  ...
  if (mask === MaskType.CHILD) {
    ...
    // Robot
    ctx.beginPath();
    ctx.arc(-eyeSep, eyeY, 5, 0, Math.PI * 2);
    ctx.stroke();
    ctx.beginPath();
    ctx.arc(eyeSep, eyeY, 5, 0, Math.PI * 2);
    ctx.stroke();
    ...
    ctx.fillStyle = "#fff";
    ctx.beginPath();
    ctx.arc(-eyeSep, eyeY, 2, 0, Math.PI * 2);
    ctx.fill();
    ctx.beginPath();
    ctx.arc(eyeSep, eyeY, 2, 0, Math.PI * 2);
    ctx.fill();
    ...
    if (isHurt) {
      ...
      ctx.beginPath();
      ctx.arc(-eyeSep + lookX, eyeY + lookY, 3, 0, Math.PI * 2);
      ctx.fill();
      ctx.beginPath();
      ctx.arc(eyeSep + lookX, eyeY + lookY, 3, 0, Math.PI * 2);
      ctx.fill();
  ...
  if (mask === MaskType.CHILD) {
    ...
    ctx.moveTo(-5 + lookX, mouthY);
    ctx.lineTo(5 + lookX, mouthY);
  } else {
    if (isHurt) ctx.arc(0, mouthY + 2, 4, 0, Math.PI * 2);
    else if (levelIdx === 4 && mask === MaskType.NONE)
      ctx.arc(0, mouthY, 5, 0, Math.PI);
    else {
      ctx.moveTo(-3 + lookX, mouthY);
      ctx.lineTo(3 + lookX, mouthY);
    }
  }
  ctx.stroke();
};
